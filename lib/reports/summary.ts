 ...
 *
 * (2026-07-26 data-email) components/app/reportSummary.ts 에서 lib 로 이관 —
 * 서버(주간 리포트 cron)와 UI(ReportsView) 가 함께 재사용하기 위한 순수 로직.
 * 경계 청결을 위해 표시 헬퍼(fmtDate/kstYmd)는 date-fns-tz(Asia/Seoul)로 이 파일에 내장한다
 * (components/app/format 과 동일 동작, lib→components 역참조 회피).
 * components/app/reportSummary.ts 는 이 파일을 재수출하는 1줄 심으로 남아 UI import 무파손.
 * buildReportSummary 는 단위 테스트 대상.
 */
...
/** ISO(임의 오프셋) → KST "7월 6일" (앞 0 없이). components/app/format.fmtDate 와 동일. */
...
/** ISO(임의 오프셋) → KST 달력 날짜 "yyyy-MM-dd". components/app/format.kstYmd 와 동일. */
