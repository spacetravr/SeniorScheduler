...
### 다음 할 일
1. **[사용자] call-dispatch 워크플로 재개 여부 결정** (실발신 트리거)
2. **[사용자] Resend 도메인 인증 or 수신자 확인** — 미인증 상태에선 가입 이메일 외 주소로 못 감. 보호자 계정이 늘면 도메인 인증 필요
3. CLOVA AiCall 트랙 재개(통화 품질 — 여전히 최대 리스크)
4. 세션 #13 논블로킹 후속 목록(그대로 유효)
