...
export default function PreregisterPage() {
  return (
    <main className="grid min-h-screen lg:grid-cols-2">
      {/* 왼쪽: 브랜드 패널 */}
      <section className="flex flex-col justify-between gap-10 bg-gradient-to-br from-surface via-surface to-accent/10 px-6 py-12 sm:px-10 lg:px-14 lg:py-16">
        <Link
          href="/"
          ...
          <ul className="flex flex-col gap-3">
            {BENEFITS.map((b) => (
              <li key={b} className="flex items-start gap-3">
                <span
                  className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-bold text-bg"
                  aria-hidden
                >
                  ✓
                </span>
