...
- 랜딩은 완성된 서비스처럼 보이지만, [사전등록하기] 버튼을 누르면 실제 가입 대신 **사전등록(대기자 이메일)** 으로 이어진다.
- 방문자가 어디까지 진행했는지(보기만 함 → 사전등록 페이지 진입 → 이메일 제출)가 전부 기록되므로, **관심의 깊이**를 단계별로 잴 수 있다.
- 사전등록 클릭 = "더 알아보고 등록하려 했다"는 신호. 이메일 제출 = 가장 강한 수요 신호.

### 랜딩 구조 (2페이지)

베타 검증 랜딩은 두 페이지로 나뉜다.

| 페이지 | 주소 | 하는 일 |
|---|---|---|
| 1페이지 (랜딩) | `/` | 서비스 소개. 방문 시 [VIEW] 1회 기록. [사전등록하기] 클릭 시 [CLICK_TRY] 기록 후 사전등록 페이지로 이동 |
| 2페이지 (사전등록) | `/preregister` | 이메일 입력·제출. 제출 시 [WAITLIST_SUBMIT] 기록 + waitlist에 이메일 저장 |

> 참고: 과거에 있던 [구독하기] 버튼(`CLICK_SUBSCRIBE`)은 UI에서 제거되어 **더 이상 발생하지 않는다.** 지표 대시보드도 이 이벤트를 집계에서 제외한다(과거 데이터가 남아 있어도 무시).

### 추적 동작 흐름

```
방문자가 링크 클릭 (utm 파라미터 포함)
  → 1페이지(/) 도착: 브라우저에 익명 ID(session_uuid) 생성 + utm 3종 저장
  → [VIEW] 이벤트 1회 기록 (새로고침해도 중복 안 됨)
  → [사전등록하기] 클릭 시 [CLICK_TRY] 기록 → 2페이지(/preregister)로 이동
  → 2페이지에서 이메일 제출 시 [WAITLIST_SUBMIT] 기록 + waitlist에 이메일 저장
```
...
| 파라미터 | 의미 | 예시 |
|---|---|---|
| `utm_source` | 어디에 뿌렸나 (채널) | `kakao`, `naver_cafe`, `instagram`, `survey`, `test` |
...
| 컬럼 | 내용 |
|---|---|
| `type` | `VIEW`(1페이지 봄) / `CLICK_TRY`(사전등록 클릭 = 2페이지 진입) / `WAITLIST_SUBMIT`(이메일 제출) |
| `session_uuid` | 익명 방문자 ID (같은 브라우저 = 같은 값) |
| `utm_source / medium / campaign` | 유입 채널 정보 |
| `created_at` | 발생 시각 |

> `CLICK_SUBSCRIBE`는 더 이상 기록되지 않는다(구독 버튼 제거). 과거 데이터가 남아 있어도 대시보드는 무시한다.
...
- 채널별 반응: 카카오 vs 네이버 카페, 어디서 온 사람이 더 많이 사전등록을 눌렀나?
- 클릭률: 방문자 중 몇 %가 사전등록 페이지까지 넘어갔나?
...
- **CLICK_TRY(사전등록 클릭)는 중복 가능**: 같은 사람이 버튼을 3번 누르면 3건이다. "몇 명이 눌렀나"가 궁금하면 대시보드의 전환율(세션 기준)과 함께 볼 것.
- **`(직접)` 유입** = utm 없이 들어온 방문 (주소 직접 입력, utm 없는 링크 공유 등). 이 비중이 크면 링크 관리가 새고 있다는 뜻.
- **`test` 소스는 집계에서 제외하고 볼 것** — 본인 테스트 데이터다. 대시보드에서도 "테스트 유입" 줄로 분리되어 합계·요약에서 빠진다.
- **`CLICK_SUBSCRIBE`(과거 구독 버튼)** 는 집계에 잡히지 않는다 — 버튼이 제거되어 신규 발생이 없고, 과거 잔존 데이터는 무시된다.
